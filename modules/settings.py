...
# ── Paleta (mismos tokens que main.py) ────────────────────────────────────────
C_BG           = "#f7f6f2"
C_SURFACE      = "#f3f0ec"
C_SURFACE_2    = "#edeae5"
C_BORDER       = "#d4d1ca"
...
def _label_hint(texto: str) -> QLabel:
    lbl = QLabel(texto)
...
class DialogoAjustes(QDialog):
    ...
    def __init__(self, config_path: Path, config: dict, parent=None):
        ...
        self.config = dict(config)  # copia local
    ...
    def _build_ui(self):
        ...
        # ── Sección: Proveedor SMTP ───────────────────────────────────────
        root.addWidget(_label_seccion("PROVEEDOR SMTP"))
        root.addSpacing(8)
        ...
        root.addWidget(
            _label_hint(
                "Elegí un proveedor para autocompletar el servidor y puerto SMTP, "
                "o elegí 'Personalizado' para ingresarlos manualmente."
            )
        )
        ...
        root.addWidget(_label_seccion("CREDENCIALES"))
        root.addSpacing(10)
        ...
        form_creds.addRow("Contraseña:", self.input_password)

        # Toggle mostrar/ocultar contraseña
        fila_pass = QHBoxLayout()
        fila_pass.setSpacing(6)
        fila_pass.addWidget(self.input_password)
        btn_toggle = QPushButton("👁")
        btn_toggle.setFixedSize(36, 36)
        btn_toggle.setProperty("secondary", "true")
        btn_toggle.style().unpolish(btn_toggle)
        btn_toggle.style().polish(btn_toggle)
        btn_toggle.setCheckable(True)
        btn_toggle.setToolTip("Mostrar / ocultar contraseña")
        ...
        fila_pass.addWidget(btn_toggle)

        # Reemplazar la fila de contraseña en el form con el layout combinado
        # (QFormLayout no soporta addRow(str, QLayout) directo en todas las versiones,
        # así que usamos un contenedor)
        form_creds.removeRow(1)
        contenedor_pass = QWidget()
        contenedor_pass.setLayout(fila_pass)
        ...
        root.addWidget(_label_seccion("SERVIDOR SMTP"))
        root.addSpacing(10)
        ...
        self.spin_puerto.setFixedWidth(90)
    ...
    def _cargar_valores(self):
        ...
        # Si el preset no es Personalizado, bloquear los campos de servidor/puerto
